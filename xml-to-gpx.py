...
import math
import logging
from datetime import datetime  # Für GPX-Metadaten
...
def generate_gpx_from_centerlines(centerline_coords, output_path):
    """Generiert eine GPX-Datei aus den absoluten Geokoordinaten der Mittellinien."""
    try:
        gpx = ET.Element("gpx", version="1.1", creator="Custom Script")
        metadata = ET.SubElement(gpx, "metadata")
        ET.SubElement(metadata, "time").text = datetime.now().isoformat()

        # Gruppieren nach polygon_id
        lane_groups = {}
        for coord in centerline_coords:
            polygon_id = coord["polygon_id"]
            if polygon_id not in lane_groups:
        ...
        # Erstellen eines Tracks für jede Fahrspur
        for lane_id, coords in lane_groups.items():
            trk = ET.SubElement(gpx, "trk")
            ET.SubElement(trk, "name").text = f"Lane_{lane_id}"
            trkseg = ET.SubElement(trk, "trkseg")
            
            for coord in coords:
                lon, lat = map(float, coord["geocoordinate"].strip("()").split(","))
                trkpt = ET.SubElement(trkseg, "trkpt", lat=f"{lat:.6f}", lon=f"{lon:.6f}")
                ET.SubElement(trkpt, "name").text = f"Point_Lane_{lane_id}_{coord['point_index']}"
                print(f"Lane {lane_id} - Punkt {coord['point_index']}: (lat={lat:.6f}, lon={lon:.6f})")
                logging.info(f"Lane {lane_id} - Punkt {coord['point_index']}: (lat={lat:.6f}, lon={lon:.6f})")

        # GPX-Datei speichern
        tree = ET.ElementTree(gpx)
        ET.indent(tree, space="  ")
        tree.write(output_path, encoding="utf-8", xml_declaration=True)
        print(f"✅ GPX-Datei gespeichert: {output_path}")
        logging.info(f"GPX-Datei gespeichert: {output_path}")

    except Exception as e:
        print(f"❌ Fehler beim Erstellen der GPX-Datei: {str(e)}")
        logging.info(f"Fehler beim Erstellen der GPX-Datei für {output_path}: {str(e)}")
        ...
        for i, polygon in enumerate(vectorized_data):
            ...
            try:
                ...
                centerline, avg_width, avg_xy = polygon_centerline(
                    polygon, dx=5.0, show_plots=False
                )
                ...
                # Generate GPX-Datei nach Speichern der centerline_coords
                gpx_path = os.path.join(output_dir_final, f"lanes_{os.path.basename(img_path).replace('.png', '.gpx')}")
                generate_gpx_from_centerlines(centerline_coords, gpx_path)
